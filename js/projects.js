...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const dataRender = (data, container) => {
    ...
    if (projectCard.length === 0) {
      ...
      container.innerHTML = '<h3 class="not-found">There are no items that satisfy the filter</h3>';
  ...
  const onClickChangeView = () => {
    ...
    inner.classList.toggle('change');
  };

  burgerBtn.addEventListener('click', onClickChangeView);
